...
from .bases import TrainImageDataset
from .bases import ValImageDataset
...
def train_collate_fn(batch):
    ...
    """
    imgs, label, index, pids, cids,_ = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)
    return torch.stack(imgs, dim=0), label, index, pids, cids
...
def make_dataset(cfg, pkl_file='index.pkl'):
    ...
    pkl = pkl_file
    train_set = TrainImageDataset(dataset.train, train_transforms, pkl_file = pkl)
    if cfg.DATASETS.QUERY_MINING:
        
        val_set = ValImageDataset(dataset.query + dataset.query, val_transforms)
    else:
        val_set = ValImageDataset(dataset.query + dataset.gallery, val_transforms)

    return train_transforms, val_transforms, dataset, train_set, val_set
...
def make_dataloader_Pseudo(cfg):
    ...
    train_set = ValImageDataset(dataset.train, train_transforms)
